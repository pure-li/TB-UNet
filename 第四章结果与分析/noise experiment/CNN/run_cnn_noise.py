...
"""CNN 噪声鲁棒性实验 — 仅推理，不重训
==========================================
噪声加在真实测线数据 FinalMag → RBF → CNN 推理
...
COMPARATIVE_CNN = r'F:\PINN实验\venv\U-net\comparative experiment\CNN_results'
...
class CNNInpainter(nn.Module):
    def __init__(self, in_ch=1, base_ch=64):
        super().__init__()
        self.enc1 = nn.Sequential(nn.Conv2d(in_ch, base_ch, 4, 2, 1), nn.LeakyReLU(0.2))
        self.enc2 = nn.Sequential(nn.Conv2d(base_ch, base_ch*2, 4, 2, 1), nn.BatchNorm2d(base_ch*2), nn.LeakyReLU(0.2))
        self.enc3 = nn.Sequential(nn.Conv2d(base_ch*2, base_ch*4, 4, 2, 1), nn.BatchNorm2d(base_ch*4), nn.LeakyReLU(0.2))
        self.enc4 = nn.Sequential(nn.Conv2d(base_ch*4, base_ch*8, 4, 2, 1), nn.BatchNorm2d(base_ch*8), nn.LeakyReLU(0.2))
        self.enc5 = nn.Sequential(nn.Conv2d(base_ch*8, base_ch*8, 4, 2, 1), nn.BatchNorm2d(base_ch*8), nn.LeakyReLU(0.2))
        self.bottleneck = nn.Conv2d(base_ch*8, base_ch*8, 1)
        self.dec1 = nn.Sequential(nn.ConvTranspose2d(base_ch*8, base_ch*8, 4, 2, 1), nn.BatchNorm2d(base_ch*8), nn.ReLU())
        self.dec2 = nn.Sequential(nn.ConvTranspose2d(base_ch*8, base_ch*4, 4, 2, 1), nn.BatchNorm2d(base_ch*4), nn.ReLU())
        self.dec3 = nn.Sequential(nn.ConvTranspose2d(base_ch*4, base_ch*2, 4, 2, 1), nn.BatchNorm2d(base_ch*2), nn.ReLU())
        self.dec4 = nn.Sequential(nn.ConvTranspose2d(base_ch*2, base_ch, 4, 2, 1), nn.BatchNorm2d(base_ch), nn.ReLU())
        self.dec5 = nn.Sequential(nn.ConvTranspose2d(base_ch, 1, 4, 2, 1), nn.Tanh())

    def forward(self, x):
        e1 = self.enc1(x); e2 = self.enc2(e1); e3 = self.enc3(e2)
        e4 = self.enc4(e3); e5 = self.enc5(e4)
        b = self.bottleneck(e5)
        d = self.dec1(b); d = self.dec2(d); d = self.dec3(d)
        d = self.dec4(d); d = self.dec5(d)
        return d
...
def cnn_inference_noise(model, data, F_display, F_grid_masked, F_min, F_max):
    model.eval()
    ...
    F_in = F_grid_masked.copy()
    F_in[np.isnan(F_in)] = 0.0
    F_in_n = norm(F_in)
    F_rs = resize(F_in_n.astype(np.float32), (TARGET_SIZE, TARGET_SIZE),
                  mode='constant', anti_aliasing=True)

    with torch.no_grad():
        inp = torch.tensor(F_rs, dtype=torch.float32).unsqueeze(0).unsqueeze(0).to(DEVICE)
        out = model(inp)
...
def copy_noise0(region_key):
    """Copy existing CNN block results for noise=0 baseline"""
    import shutil
    src_files = [
        (f'{COMPARATIVE_CNN}/result_grid_{region_key}.npy', f'result_grid_{region_key}_noise_0.npy'),
        (f'{COMPARATIVE_CNN}/truth_grid_{region_key}.npy', f'truth_grid_{region_key}.npy'),
        (f'{COMPARATIVE_CNN}/mask_blank_{region_key}.npy', f'mask_blank_{region_key}.npy'),
        (f'{COMPARATIVE_CNN}/mask_outside_{region_key}.npy', f'mask_outside_{region_key}.npy'),
        (f'{COMPARATIVE_CNN}/grid_x_{region_key}.npy', f'grid_x_{region_key}.npy'),
        (f'{COMPARATIVE_CNN}/grid_y_{region_key}.npy', f'grid_y_{region_key}.npy'),
        (f'{COMPARATIVE_CNN}/bx_{region_key}.npy', f'bx_{region_key}.npy'),
        (f'{COMPARATIVE_CNN}/by_{region_key}.npy', f'by_{region_key}.npy'),
    ]
    ...
    with open(f'{COMPARATIVE_ROOT}/comparison_metrics.json') as f:
        cm = json.load(f)
    cnn_r = cm[region_key].get('CNN (Block)', cm[region_key].get('CNN'))
    result_dict = {
        'method': 'CNN', 'region': region_key, 'noise_sigma': 0,
        'rmse': cnn_r['rmse'], 'mae': cnn_r['mae'],
        'time': 0, 'status': 'completed',
        'timestamp': time.strftime('%Y-%m-%d %H:%M:%S'),
    }
    with open(os.path.join(OUT_DIR, f'results_{region_key}_noise_0.json'), 'w') as f:
        json.dump(result_dict, f, indent=2)
    print(f"  [{region_key}] σ=0nT (reused) → RMSE={cnn_r['rmse']:.2f}, MAE={cnn_r['mae']:.2f}")
...
print("=" * 60)
print("  CNN 噪声鲁棒性实验 (仅推理)")
print("=" * 60)
print(f"  Device: {DEVICE}")

# Load CNN model (block mask version)
print("\nLoading CNN model...")
model = CNNInpainter(in_ch=1, base_ch=64).to(DEVICE)
ckpt = torch.load(os.path.join(COMPARATIVE_CNN, 'cnn_block_model.pt'), map_location=DEVICE)
model.load_state_dict(ckpt)
model.eval()
print("  CNN block model loaded.")
...
for rk, (rlabel, poly, rect) in regions.items():
    ...
    for sigma in [2, 5, 10]:
        ...
        result_grid = cnn_inference_noise(model, data, F_display, F_grid_masked, F_min, F_max)
        ...
        result = {
            'method': 'CNN', 'region': rk, 'noise_sigma': sigma,
...
print(f"\n{'='*60}")
print(f"  CNN 噪声实验完成! 总耗时: {progress['total_time_min']} min")
print("=" * 60)
