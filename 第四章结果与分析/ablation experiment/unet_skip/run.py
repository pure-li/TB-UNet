#!/usr/bin/env python
"""消融实验 — U-Net + Skip (有跳跃连接, 无Transformer)
=====================================================
架构: 传统U-Net, Encoder-Bottleneck(Conv)-Decoder + 跳跃连接
...
print("=" * 60)
print("  Ablation: U-Net + Skip (with skip, no transformer)")
print(f"  epochs={EPOCHS}, max_lr={MAX_LR}, pct_start={PCT_START}, block mask")
print("=" * 60)

# =============================================================================
# U-Net + Skip: Encoder-Bottleneck-Decoder WITH skip connections
# =============================================================================
class UNetSkip(nn.Module):
    def __init__(self, in_chans=1, base_ch=48):
        ...
        self.bottleneck = unet.DoubleConv(bc * 4, bc * 4)
...
def train_model(data, prep, region_key):
    print(f"\n[U-Net + Skip] Training {region_key}, epochs={EPOCHS}")
    np.random.seed(SEED); torch.manual_seed(SEED)
    if torch.cuda.is_available(): torch.cuda.manual_seed_all(SEED)

    model = UNetSkip(in_chans=1, base_ch=BASE_CH).to(DEVICE)
...
summary['config'] = {'model': 'U-Net + Skip (with skip, no transformer)',
