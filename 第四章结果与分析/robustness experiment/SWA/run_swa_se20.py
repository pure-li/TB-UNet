#!/usr/bin/env python
"""Robustness: SWA on 2.0° SE region — 100 epochs, top-3 checkpoint averaging
Region: 64.5-66.5°E, 30.0-32.0°N (2.0° × 2.0°, SE)"""
...
CKPT_DIR = os.path.join(OUT_DIR, 'checkpoints_se20')
...
RECT = (64.5, 66.5, 30.0, 32.0)  # 2.0° × 2.0° SE
REGION_KEY = 'se20'

print("=" * 60)
print(f"  SWA 鲁棒性实验 — 2.0° SE region")
print(f"  RECT: {RECT}")
...
results = {
    REGION_KEY: {
        'best_rmse': best_rmse, 'best_mae': best_mae, 'best_ep': best_ep,
        'swa_rmse': swa_rmse, 'swa_mae': swa_mae,
        'swa_epochs': selected, 'train_time': train_time,
    },
    'config': {
        'mode': 'swa', 'region': '2.0deg SE',
        'rect': RECT, 'epochs': EPOCHS, 'max_lr': MAX_LR, 'pct_start': PCT_START,
        'base_ch': BASE_CH, 'batch_size': BATCH_SIZE, 'n_params': n_params,
    }
}
