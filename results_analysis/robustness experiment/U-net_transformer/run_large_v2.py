#!/usr/bin/env python
"""U-Net+Transformer 大区域参数调优 (Large, 1.5deg SE)
=======================================================
epochs=200, max_lr=2e-4, pct_start=0.5, wd=1e-4
"""
...
MAX_LR = 2e-4; PCT_START = 0.5; WEIGHT_DECAY = 1e-4
N_INTERP_CTRL = 3000; TARGET_SIZE = (128, 128)

REGION = {'key': 'large', 'label': 'Large (1.5deg, SE)', 'rect': (63.0, 64.5, 30.5, 32.0)}

print("=" * 60)
print("  U-Net+Transformer — Large v2 (参数调优)")
...
for ep in range(1, EPOCHS + 1):
    ...
    # Early stopping: no improvement for 60 epochs (wider window for large)
    if ep - best_ep > 60:
        print(f"  Early stop at epoch {ep} (no improvement for 60 epochs)")
        break
...
# Save (overwrite large_v2)
torch.save(best_state, os.path.join(OUT_DIR, f'best_model_large.pt'))
np.save(os.path.join(OUT_DIR, f'result_grid_large.npy'), best_result_grid)
np.save(os.path.join(OUT_DIR, f'truth_grid_large.npy'), truth_grid)
np.save(os.path.join(OUT_DIR, f'mask_blank_large.npy'), data['mask_blank'])
np.save(os.path.join(OUT_DIR, f'mask_outside_large.npy'), data['mask_outside'])
np.save(os.path.join(OUT_DIR, f'grid_x_large.npy'), data['grid_x'])
np.save(os.path.join(OUT_DIR, f'grid_y_large.npy'), data['grid_y'])
np.save(os.path.join(OUT_DIR, f'bx_large.npy'), data['bx'])
np.save(os.path.join(OUT_DIR, f'by_large.npy'), data['by'])
with open(os.path.join(OUT_DIR, f'history_large.json'), 'w') as f:
...
results['large'] = {
    ...
    'config': {'max_lr': MAX_LR, 'pct_start': PCT_START, 'epochs': EPOCHS, 'weight_decay': WEIGHT_DECAY},
}

results['config'] = results.get('config', {})
results['config']['large_v2'] = {'max_lr': MAX_LR, 'pct_start': PCT_START, 'epochs': EPOCHS, 'weight_decay': WEIGHT_DECAY}
...
ax.set_title(f'{rlabel} (v2) — Test RMSE')
ax.grid(True, alpha=0.3)
save_fig(fig, f'fig_rmse_large_v2')

plt.close('all')

print(f"\n{'='*60}")
print(f"  Large v2 完成! RMSE={best_rmse:.2f}, MAE={mae:.2f}")
print(f"  输出: {OUT_DIR}/")
print("=" * 60)
