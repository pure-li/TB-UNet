#!/usr/bin/env python
"""鲁棒性实验 — U-Net+TF (slow训练方式) + Kriging
====================================================
两个区域:
  Small:  61.5-62.0°E, 34.0-34.5°N (0.5° × 0.5°, NW)
  Large:  63.0-64.5°E, 30.5-32.0°N (1.5° × 1.5°, SE)
...
# 两个区域
REGIONS = {
    'small': ('Small (0.5deg x 0.5deg, NW)', (61.5, 62.0, 34.0, 34.5)),
    'large': ('Large (1.5deg x 1.5deg, SE)', (63.0, 64.5, 30.5, 32.0)),
}

print("=" * 60)
print("  鲁棒性实验 — U-Net+TF (slow) + Kriging — 两个区域")
...
def train_unet_slow(data, region_key):
    prep = preprocess(data)

    # Reset seeds before each training
    ...
    return {'best_state': best_state, 'best_result_grid': best_result_grid,
            'best_rmse': best_rmse, 'mae': mae, 'best_epoch': best_ep,
...
for rk, (rlabel, rect) in REGIONS.items():
    print(f"\n{'#'*60}")
    print(f"# {rlabel} — Region: {rect}")
    print(f"{'#'*60}")
    ...
    # Save Kriging
    np.save(os.path.join(KRIG_DIR, f'result_grid_{rk}.npy'), krig_result)
    np.save(os.path.join(KRIG_DIR, f'truth_grid_{rk}.npy'), truth_grid)
    np.save(os.path.join(KRIG_DIR, f'mask_blank_{rk}.npy'), data['mask_blank'])
    np.save(os.path.join(KRIG_DIR, f'mask_outside_{rk}.npy'), data['mask_outside'])
    np.save(os.path.join(KRIG_DIR, f'grid_x_{rk}.npy'), data['grid_x'])
    np.save(os.path.join(KRIG_DIR, f'grid_y_{rk}.npy'), data['grid_y'])
    np.save(os.path.join(KRIG_DIR, f'bx_{rk}.npy'), data['bx'])
    np.save(os.path.join(KRIG_DIR, f'by_{rk}.npy'), data['by'])
    krig_results[rk] = {'rmse': float(krig_rmse), 'mae': float(krig_mae), 'time': krig_time,
    ...
    result = train_unet_slow(data, rk)
    best_rmse = result['best_rmse']; mae = result['mae']; best_ep = result['best_epoch']
    train_time = result['train_time']

    # Save U-Net+TF
    torch.save(result['best_state'], os.path.join(UNET_DIR, f'best_model_{rk}.pt'))
    np.save(os.path.join(UNET_DIR, f'result_grid_{rk}.npy'), result['best_result_grid'])
    np.save(os.path.join(UNET_DIR, f'truth_grid_{rk}.npy'), truth_grid)
    np.save(os.path.join(UNET_DIR, f'mask_blank_{rk}.npy'), data['mask_blank'])
    np.save(os.path.join(UNET_DIR, f'mask_outside_{rk}.npy'), data['mask_outside'])
    np.save(os.path.join(UNET_DIR, f'grid_x_{rk}.npy'), data['grid_x'])
    np.save(os.path.join(UNET_DIR, f'grid_y_{rk}.npy'), data['grid_y'])
    np.save(os.path.join(UNET_DIR, f'bx_{rk}.npy'), data['bx'])
    np.save(os.path.join(UNET_DIR, f'by_{rk}.npy'), data['by'])
    with open(os.path.join(UNET_DIR, f'history_{rk}.json'), 'w') as f:
        json.dump(result['history'], f, indent=2)

    unet_results[rk] = {'rmse': float(best_rmse), 'mae': float(mae),
    ...
    # Keep medium if present
    if 'medium' not in unet_results:
        unet_results['medium'] = {'rmse': 19.92, 'mae': 15.46, 'best_epoch': 35,
                                   'source': 'reused from U-net_transformer_slow'}
    with open(unet_results_path, 'w') as f: json.dump(unet_results, f, indent=2, ensure_ascii=False)

    print(f"\n  {rk}: U-Net+TF={best_rmse:.2f}, Kriging={krig_rmse:.2f}, "
          f"Gap: {krig_rmse - best_rmse:+.1f} nT")

total_time = time.time() - all_start
print(f"\n{'='*60}")
print(f"  全部完成! 总耗时: {total_time/60:.0f} min")
print(f"  Small:  U-Net+TF={unet_results.get('small',{}).get('rmse','?'):.2f}, "
      f"Kriging={krig_results.get('small',{}).get('rmse','?'):.2f}")
print(f"  Large:  U-Net+TF={unet_results.get('large',{}).get('rmse','?'):.2f}, "
      f"Kriging={krig_results.get('large',{}).get('rmse','?'):.2f}")
print("=" * 60)
