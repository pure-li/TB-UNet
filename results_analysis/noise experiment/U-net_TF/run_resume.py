#!/usr/bin/env python
"""U-Net+TF 噪声实验续跑 — 跳过已完成的 rect noise=0,2
========================================================
"""
...
# Tasks to run: (region_key, sigma_list)
TASKS = [
    ('rect', [5, 10]),
    ('irreg', [0, 2, 5, 10]),
]
...
def copy_noise0(region_key):
    ...
    if os.path.exists(os.path.join(OUT_DIR, f'result_grid_{tag}.npy')):
        print(f"    [{tag}] already exists, skipping copy")
        return
    src_files = [
    ...
    print(f"    [{region_key}] σ=0nT (reused) → computed abs_residual")
...
def save_shared_data(rk, data, truth_grid):
...
for rk, sigma_list in TASKS:
    rlabel, poly, rect = regions_config[rk]
    print(f"\n{'#'*60}")
    print(f"# Region: {rlabel}")
    print(f"{'#'*60}")

    # Load shared data FIRST (needed by copy_noise0)
    data = load_region_data(poly_vertices=poly, rect_bounds=rect)
    truth_grid = compute_truth_grid(data)
    save_shared_data(rk, data, truth_grid)

    # Copy noise=0 if needed
    if 0 in sigma_list:
        copy_noise0(rk)
    ...
    for sigma in sigma_list:
        ...
        if os.path.exists(os.path.join(OUT_DIR, f'results_{tag}.json')):
            print(f"\n  [{tag}] already completed, skipping")
            continue
...
print(f"\n{'='*60}")
print(f"  全部完成! 总耗时: {(time.time()-run_start)/60:.0f} min")
print(f"{'='*60}")
