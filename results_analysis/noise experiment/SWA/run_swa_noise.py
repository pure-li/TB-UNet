#!/usr/bin/env python
"""U-Net+TF+SWA 噪声鲁棒性实验
==========================================================
噪声加在训练数据 → RBF背景场 → 自监督训练(100 epochs) → SWA平均
σ = 2, 5, 10 nT, rect & irreg
"""
...
print("=" * 60)
print("  U-Net+TF+SWA 噪声鲁棒性实验")
print(f"  Epochs={EPOCHS}, lr={MAX_LR}, σ=2/5/10 nT")
print(f"  Device: {DEVICE}")
print("=" * 60)


# =============================================================================
# Model (mirror run_experiment.py)
# =============================================================================
...
# =============================================================================
# Data loading (mirror noise experiment)
# =============================================================================
...
def preprocess_with_noise(data, sigma):
    np.random.seed(SEED)
    ...
    return {'F_display': F_display, 'F_min': F_min, 'F_max': F_max,
            ...
            'norm': norm, 'denorm': denorm, 'eps': eps}
...
def evaluate(model, test_input_t, zf, data, prep):
    ...
    return float(rmse), float(mae), rg, abs_res


# =============================================================================
# SWA Training
# =============================================================================
def train_swa(data, prep, region_key, sigma):
    ...
    ckpt_dir_region = os.path.join(CKPT_DIR, tag)
    os.makedirs(ckpt_dir_region, exist_ok=True)
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        if ep % 10 == 0:
            torch.save({
                ...
                'rmse': rmse, 'mae': mae,
            }, os.path.join(ckpt_dir_region, f'ep{ep:03d}.pt'))
    ...
    swa_state = None
    for ep in selected:
        ckpt = torch.load(os.path.join(ckpt_dir_region, f'ep{ep:03d}.pt'),
    ...
    return {'best_rmse': best_rmse, 'best_ep': best_ep, 'best_mae': best_mae,
            'best_state': best_state, 'best_grid': best_grid, 'best_res': best_res,
            'swa_rmse': swa_rmse, 'swa_mae': swa_mae, 'swa_state': swa_state,
            'swa_grid': swa_grid, 'swa_res': swa_res, 'swa_epochs': selected,
            'train_time': train_time, 'n_params': n_params, 'history': history}
...
# Copy shared data (truth_grid, mask_blank, grid_x, etc.) from original U-net_TF noise experiment
SRC_NOISE = r'F:\PINN实验\venv\U-net\noise experiment\U-net_TF'
import shutil
for rk in ['rect', 'irreg']:
    for fname in [f'truth_grid_{rk}.npy', f'mask_blank_{rk}.npy', f'mask_outside_{rk}.npy',
                  f'grid_x_{rk}.npy', f'grid_y_{rk}.npy', f'bx_{rk}.npy', f'by_{rk}.npy']:
        src = os.path.join(SRC_NOISE, fname)
        dst = os.path.join(OUT_DIR, fname)
        if os.path.exists(src) and not os.path.exists(dst):
            shutil.copy2(src, dst)
            print(f"  Copied: {fname}")

# Copy σ=0 results from SWA experiment (补充实验)
SRC_SWA = r'F:\PINN实验\venv\U-net\补充实验\SWA'
for rk in ['rect', 'irreg']:
    for src_f, dst_f in [
        (f'result_grid_swa_{rk}.npy', f'result_grid_swa_{rk}_noise_0.npy'),
        (f'abs_residual_swa_{rk}.npy', f'abs_residual_swa_{rk}_noise_0.npy'),
        (f'result_grid_{rk}.npy', f'result_grid_{rk}_noise_0.npy'),
        (f'abs_residual_{rk}.npy', f'abs_residual_{rk}_noise_0.npy'),
        (f'history_{rk}.json', f'history_{rk}_noise_0.json'),
    ]:
        src_path = os.path.join(SRC_SWA, src_f)
        dst_path = os.path.join(OUT_DIR, dst_f)
        if os.path.exists(src_path) and not os.path.exists(dst_path):
            shutil.copy2(src_path, dst_path)
            print(f"  Copied: {src_f} → {dst_f}")

    # Copy results.json info for σ=0
    swa_res_json = os.path.join(SRC_SWA, 'results.json')
    if os.path.exists(swa_res_json):
        with open(swa_res_json) as f:
            swa_r = json.load(f)
        if rk in swa_r:
            res0 = {'method': 'U-Net+TF+SWA', 'region': rk, 'noise_sigma': 0,
                    'best_rmse': swa_r[rk].get('best_rmse'),
                    'swa_rmse': swa_r[rk].get('swa_rmse'),
                    'status': 'reused'}
            with open(os.path.join(OUT_DIR, f'results_{rk}_noise_0.json'), 'w') as f:
                json.dump(res0, f, indent=2)
            print(f"  σ=0 {rk}: best={res0['best_rmse']}, SWA={res0['swa_rmse']}")

run_start = time.time()

for rk, (rlabel, poly, rect) in regions.items():
    print(f"\n{'#'*60}")
    print(f"# Region: {rlabel} ({rk})")
    print(f"{'#'*60}")
    ...
    for sigma in [2, 5, 10]:
        ...
        print(f"\n  --- σ = {sigma} nT ---")
        prep = preprocess_with_noise(data, sigma)
        print(f"  eps={prep['eps']:.4f}")

        result = train_swa(data, prep, rk, sigma)

        # Save
        np.save(os.path.join(OUT_DIR, f'result_grid_swa_{tag}.npy'), result['swa_grid'])
        np.save(os.path.join(OUT_DIR, f'result_grid_{tag}.npy'), result['best_grid'])
        np.save(os.path.join(OUT_DIR, f'abs_residual_swa_{tag}.npy'), result['swa_res'])
        np.save(os.path.join(OUT_DIR, f'abs_residual_{tag}.npy'), result['best_res'])
        torch.save(result['best_state'], os.path.join(OUT_DIR, f'best_model_{tag}.pt'))
        torch.save(result['swa_state'], os.path.join(OUT_DIR, f'best_model_swa_{tag}.pt'))

        with open(os.path.join(OUT_DIR, f'history_{tag}.json'), 'w') as f:
            json.dump(result['history'], f, indent=2)

        res_dict = {'method': 'U-Net+TF+SWA', 'region': rk, 'noise_sigma': sigma,
                    'best_rmse': result['best_rmse'], 'best_ep': result['best_ep'],
                    'swa_rmse': result['swa_rmse'], 'swa_epochs': result['swa_epochs'],
                    'train_time': result['train_time'], 'status': 'completed'}
        with open(os.path.join(OUT_DIR, f'results_{tag}.json'), 'w') as f:
            json.dump(res_dict, f, indent=2)

        print(f"  Saved: best={result['best_rmse']:.2f}, SWA={result['swa_rmse']:.2f}")

elapsed = (time.time() - run_start) / 60
print(f"\n{'='*60}")
print(f"  All done! Total: {elapsed:.0f} min")
print(f"{'='*60}")
