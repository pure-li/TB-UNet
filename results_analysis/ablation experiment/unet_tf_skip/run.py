#!/usr/bin/env python
"""消融实验 — U-Net + Transformer + Skip (Transformer瓶颈 + 跳跃连接)
=====================================================================
架构: U-Net编码器-Transformer瓶颈-解码器 + 跳跃连接
...
print("=" * 60)
print("  Ablation: U-Net + Transformer + Skip")
...
# =============================================================================
# U-Net + Transformer + Skip
# =============================================================================
class UNetTransformerSkip(nn.Module):
    def __init__(self, in_chans=1, base_ch=48):
        ...
        self.transformer = TransformerBottleneck(
            in_ch=bc * 4, spatial_size=8, num_layers=4, num_heads=8, mlp_ratio=4, dropout=0.1)
...
def train_model(data, prep, region_key):
    print(f"\n[U-Net + TF + Skip] Training {region_key}, epochs={EPOCHS}")
    np.random.seed(SEED); torch.manual_seed(SEED)
    if torch.cuda.is_available(): torch.cuda.manual_seed_all(SEED)

    model = UNetTransformerSkip(in_chans=1, base_ch=BASE_CH).to(DEVICE)
    ...
    for ep in range(1, EPOCHS+1):
        ...
        # Early stopping disabled — run full 100 epochs
...
summary['config'] = {'model': 'U-Net + Transformer + Skip',
